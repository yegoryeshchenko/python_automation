...
def get_address():
    ...
    with open(full_path('data.csv'), 'r') as file:
        addresses_file = csv.reader(file)
        all_addresses = list(addresses_file)

        for i in range(5, 12):
            valid_addresses = []
            for address in all_addresses:
                try:
                    if len(address) > i and address != '':
                        valid_addresses.append(address[i])
                except IndexError:
                    print('Number of columns is invalid!')
                if valid_addresses:
                    full_address.append(choice(valid_addresses))
                else:
                    full_address.append("Invalid address")
    full_address = dict(zip(address_params, full_address))
    return full_address
